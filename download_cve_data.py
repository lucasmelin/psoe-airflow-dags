...
def combine_cve_results(cve_jsons):
    ...
    for json in cve_jsons:
        for result in json["results"]:
            all_cves.append(
                [result["id"], result["cvss"], result["summary"], result["Modified"]]
            )
    return all_cves
...
def get_product_cves():
    ...
    all_products = [
        ...
        Product("microsoft", "skype_for_business"),
    ]
    cve_results = []
...
def csv_upload_to_blob():
    try:
        ...
        block_blob_service = BlockBlobService(
            account_name=connection.login, account_key=connection.password
        )
        # Create Container
...
with DAG(
    "export_cve_data",
    description="Export CVE data",
    schedule_interval="@once",
    start_date=datetime.now() - timedelta(hours=24),
) as dag:
    export_task = PythonOperator(
        task_id="export_task", python_callable=get_product_cves
    )
